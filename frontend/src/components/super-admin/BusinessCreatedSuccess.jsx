...
// ✅ Iframe-based print — no popup window, no browser permission prompt
...
<head>
  ...
  <style>
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
    ...
    .hdr{background:linear-gradient(135deg,#0f172a 0%,#1e3a5f 100%);padding:36px 40px 28px;position:relative;overflow:hidden}
    .hdr::before{content:'';position:absolute;inset:0;background:repeating-linear-gradient(45deg,transparent,transparent 24px,rgba(255,255,255,.025) 24px,rgba(255,255,255,.025) 25px)}
    .badge{display:inline-flex;align-items:center;gap:6px;background:rgba(5,150,105,.2);border:1px solid rgba(5,150,105,.4);color:#6ee7b7;font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;padding:4px 12px;border-radius:99px;margin-bottom:14px}
    .title{font-family:'Playfair Display',Georgia,serif;font-size:28px;font-weight:700;color:#fff;line-height:1.15;margin-bottom:6px}
    ...
    .trial{background:#ecfdf5;border:1.5px solid #6ee7b7;border-radius:10px;padding:16px 20px;display:flex;gap:14px;align-items:flex-start;margin-bottom:28px}
    .trial-icon{width:38px;height:38px;flex-shrink:0;background:#059669;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:18px}
    ...
    .sh{font-size:11px;font-weight:600;letter-spacing:.1em;text-transform:uppercase;color:#94a3b8;padding-bottom:8px;border-bottom:1px solid #e2e8f0;margin:28px 0 14px}
    ...
    .hl{display:inline-block;background:#fef9c3;padding:2px 8px;border-radius:4px;font-family:'JetBrains Mono',monospace;font-size:15px;font-weight:700;color:#92400e;letter-spacing:.06em}
    .warn{background:#fffbeb;border:1px solid #fde68a;border-radius:8px;padding:11px 14px;font-size:12px;color:#92400e;margin-bottom:12px}
...
<div class="sheet">
  ...
  <div class="body">
    ...
    <div class="warn">⚠️ The temporary password below is shown <strong>only once</strong>. Share it securely and ask the owner to change it on first login.</div>
    <div class="grid">
  ...
  function runPrint() {
    if (printed) return;
    printed = true;
    try { cw.focus(); cw.print(); } catch (e) { console.warn('print failed', e); }
    setTimeout(() => {
  ...
  return (
    <div style={S.field}>
      ...
      <div style={S.fieldRow}>
        <span style={{ ...S.fieldValue, ...(mono ? S.mono : {}), ...(highlight ? S.highlight : {}) }}>
          {value || '—'}
        </span>
...
const S = {
  overlay:    { position:'fixed',inset:0,zIndex:9999,background:'rgba(15,23,42,.72)',backdropFilter:'blur(6px)',display:'flex',alignItems:'center',justifyContent:'center',padding:24,overflowY:'auto' },
  sheet:      { background:'#fff',borderRadius:16,width:'100%',maxWidth:680,boxShadow:'0 32px 80px rgba(0,0,0,.28)',overflow:'hidden',fontFamily:"'DM Sans',sans-serif",color:'#0f172a',position:'relative' },
  header:     { background:'linear-gradient(135deg,#0f172a 0%,#1e3a5f 100%)',padding:'36px 40px 28px',position:'relative',overflow:'hidden' },
  closeBtn:   { position:'absolute',top:16,right:16,background:'rgba(255,255,255,.08)',border:'none',cursor:'pointer',width:32,height:32,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',color:'#94a3b8' },
  badge:      { display:'inline-flex',alignItems:'center',gap:6,background:'rgba(5,150,105,.2)',border:'1px solid rgba(5,150,105,.4)',color:'#6ee7b7',fontSize:11,fontWeight:600,letterSpacing:'.08em',textTransform:'uppercase',padding:'4px 12px',borderRadius:99,marginBottom:14 },
  title:      { fontFamily:"'Playfair Display',Georgia,serif",fontSize:26,fontWeight:700,color:'#fff',margin:'0 0 6px',lineHeight:1.15 },
  ...
  trialBanner:{ background:'linear-gradient(135deg,#ecfdf5,#d1fae5)',border:'1.5px solid #6ee7b7',borderRadius:10,padding:'16px 20px',display:'flex',gap:14,alignItems:'flex-start',marginBottom:28 },
  ...
  sectionHead:{ display:'flex',alignItems:'center',gap:8,fontSize:11,fontWeight:600,letterSpacing:'.1em',textTransform:'uppercase',color:'#94a3b8',paddingBottom:8,borderBottom:'1px solid #e2e8f0',marginBottom:14,marginTop:24 },
        ...
        <motion.div style={S.overlay} initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}>
          ...
          >
            <div style={S.header}>
              ...
              <div style={S.issuedBox}><span style={S.issuedLabel}>Issued</span>{issuedAt}</div>
              <div style={S.badge}><CheckCircle size={12} /> Business Created</div>
              <h1 style={S.title}>{business.businessName}</h1>
              <p style={S.subtitle}>Platform credentials — keep this information secure</p>
            </div>

            <div style={S.body}>
              {subscription?.isTrial && (
                <div style={S.trialBanner}>
                  <div style={S.trialIcon}><Gift size={18} color="#fff" /></div>
                  <div>
                    <p style={S.trialTitle}>14-Day Free Trial Active</p>
                    <p style={S.trialDesc}>Full access to all platform features — no payment required.</p>
                    <div style={S.trialDates}>
                      <div style={S.trialDate}><span style={S.trialDateLabel}>Started</span>{issuedAt}</div>
                      <div style={S.trialDate}><span style={S.trialDateLabel}>Expires</span>{trialEnd}</div>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div style={S.grid}>
                ...
                {business.description  && <div style={S.full}><Field label="Description" value={business.description} /></div>}
              </div>
              ...
              <div style={S.grid}>
                ...
                <div style={S.full}><Field label="Email (Login)"      value={admin.email}            mono /></div>
                <div style={S.full}><Field label="Temporary Password" value={admin.temporaryPassword} mono highlight /></div>
              </div>
            </div>

            <div style={S.actions}>
              <button style={S.btnPrimary} onClick={() => printViaIframe({ business, admin, subscription })}>
                <Printer size={15} /> Print Credentials
              </button>
              <button style={S.btnOutline} onClick={onClose}>Done</button>
            </div>
          </motion.div>
        </motion.div>
      </AnimatePresence>
